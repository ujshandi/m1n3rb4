...
class Sys_login_model extends CI_Model{
	...
	function cek_login($username,$passwd){
		...
		$this->db->select('u.user_name,u.full_name, u.passwd,u.group_id,u.user_id,g.app_type, l.level,l.level_id');
		...
		if ($query->num_rows() > 0) {
			...
			$this->create_session($row['user_id'], $row['user_name'], $row['full_name'],true,$row['level'],$row['group_id'],$row['level_id']);
			$query->free_result();
	...
	private function create_session($user_id, $user_name,$full_name,$logged_in,$level,$group_id,$level_id) {
		$this->session->set_userdata('user_id',$user_id);
		$this->session->set_userdata('group_id',$user_id);//biar ga ngerubah banyak
		$this->session->set_userdata('user_name',$user_name);
		
		$this->session->set_userdata('full_name',$full_name);		
		$this->session->set_userdata('logged_in_e_tracking',$logged_in);		
	...
	private function insertLoginLog($data){
		...
		$this->db->set('user_info','id='.$data['user_id'].';name='.$data['user_name'].';');
